...
Object3D.userDataParent = null;
Mesh.userDataParent = null;

class EventsControls {
  ...
  var _this=this;
	...
	this._raySet = function () {
		...
		if ( _this.camera instanceof OrthographicCamera ) {
			...
			var vector = new Vector3( _this._mouse.x, _this._mouse.y, 1 );
			//_this._projector.unprojectVector( vector, camera ); 
			vector.unproject( _this.camera );
			//	_this.raycaster = new Raycaster( _this.camera.position, vector.sub( _this.camera.position ).normalize() );
	...
	function onContainerMouseDown( event ) {
        ...
        _this.event.buttons = event.buttons;
				...
				if ( _dragAndDropFlag ) {
					...
					try {
						if ( _this.offsetUse ) { 
							var pos = new Vector3().copy( _this.focused.position );		
	...
	function onContainerMouseMove() {
		...
		if ( _this.focused ) {
			...
			if ( _dragAndDropFlag ) {
				...
				//_this._setMap();
				try {
					var pos = new Vector3().copy( _DisplaceIntersectsMap[ 0 ].point.sub( _this.offset ) );			
